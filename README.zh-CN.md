...
Coollama 重点解决很多 Ollama 桌面使用场景里的一个实际痛点：**思考开关**。

一些推理模型在 Ollama Desktop 类客户端中会持续开启思考；当思考过程无法关闭时，日常短提示词也会明显变慢，效率下降。Coollama 为所有支持思考的模型提供独立的 **Thinking / 思考** 开关，你可以随时决定是否启用推理过程。

复杂任务时打开思考，获得更充分的推理；普通聊天、翻译、总结、简单代码问题时关闭思考，获得更快响应。

## 特性

- 自动获取并切换本地 Ollama 模型
- 流式对话与 Markdown 块级渲染
- 思考开关：所有支持思考的模型都可以随时打开或关闭思考
- 思考过程展示：透传 Ollama `think` 参数，并在可折叠区域展示推理过程
